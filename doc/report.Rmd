...
Noticed that because the confidence intervals (difference between dash lines of same color) for both groups and skills do not overlap, we should expect a significant difference in mean aggregate scores for both skills between groups when we run a T-test statistics.
...
Histograms and confidence intervals for both FSA skills and groups (aboriginal vs non-aboriginal) were constructed in the same way as with the independent vs public shools for consistency purposes. Results on the plots also indicate that there is a significant difference in scores between the two student's subgroups for both numeracy and reading skills.
